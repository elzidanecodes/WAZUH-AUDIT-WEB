    ...
    <div class="card p-4 shadow-sm">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
                ...
                <form method="GET" action="{{ route('log') }}" class="mb-4">
                    ...
                    @forelse ($logs as $log)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ ($logs->firstItem() ?? 0) + $loop->index }}
                            </td>
                            <td class="px-6 py-4">{{ $log->timestamp ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $log->host ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $log->source ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $log->process ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $log->message ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <button onclick="showFullLog('{{ $log->id }}')"
                                    class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-1.5 dark:bg-blue-500 dark:hover:bg-blue-600">
                                    Detail
                                </button>
                            </td>
                        </tr>
